...
amh_CoVaRUp<-function(level.CoVaR=level.CoVaR,level.VaR=level.VaR,par=par)
{
  cond=NULL
  par=as.matrix(par)
  N=length(par)
  for (i in 1:N){
    CoVaR=function(x) pCopula(c(x,level.VaR), amhCopula(par[i]))-((1-level.CoVaR)*(1-level.VaR))
    cond[i]=fzero(CoVaR,0, tol = 10^-16)$x
  }
  cond
}
