...
data.file   = c('CDS_Data_VIX.xlsx','CDS_Data_usfcon.xlsx')[2]          #<<<--- String con el nombre del archivo de datos en la carpeta de trabajo.
...
if (1) ARMA.Order = ARMA.ORDER.DF(Datos=DATOS_WNA)
...
#Gráficas
if (is.null(CoVaR.plot)==FALSE){
  load('CoVaR_data_Vix')
  load('CoVaR_data_usfcon')
  #CoVaR_data          = CoVaR_data_Vix; Serie2='vix'
  CoVaR_data          = CoVaR_data_usfcon; Serie2='usfcon'
  CoVaR.series        = xts(CoVaR_data$CoVaR)
  CoVaRUp.series      = xts(CoVaR_data$CoVaRUp) 
  VaR.series          = xts(CoVaR_data$VaR)
  VaRUp.series        = xts(CoVaR_data$VaRUp)
  DeltaCoVaR.series   = xts(CoVaR_data$DeltaCoVaR)
  DeltaCoVaRUp.series = xts(CoVaR_data$DeltaCoVaRUp)
  CoVaRMedian.series  = xts(CoVaR_data$CoVaRMedian)  
  x11()
  par(mfrow=c(4,3))
  for (i in Name.Series1){
    #par(mfrow=c(4,3))
    #pdf(file = paste0(Resultados,'/Graficas_CoVaR_',i,'.pdf'), onefile=FALSE)
    #pdf(file = paste0(Resultados,'/Graficas_CoVaR.pdf'), onefile=FALSE)
    ...
    plot.xts((if (CoVaR.plot=='Down'|CoVaR.plot=='Both') CoVaR.series[,i]
                    else if (CoVaR.plot=='Up')  CoVaRUp.series[,i]),
                   type="l",col="red", ylim=rango,xlab="Time",
                   ylab="", lwd=1.5, main=paste(i,'-',Serie2), format.labels="%Y", major.ticks = 'years', 
                   yaxis.left=TRUE, yaxis.right=TRUE, lty='dotted')
    if ((CoVaR.plot=='Down'|CoVaR.plot=='Both') & VaR.plot=='TRUE')print(lines(VaR.series[,i],        col="black", lwd=1.5,lty='dashed'))
    if (CoVaR.plot=='Down'|CoVaR.plot=='Both')                     print(lines(DeltaCoVaR.series[,i], col="blue",  lwd=1.5,lty='solid'))
    if (CoVaR.plot=='Both')                                        print(lines(CoVaRUp.series[,i],    col="red",   lwd=1.5,lty='dotted'))
    if ((CoVaR.plot=='Up'|CoVaR.plot=='Both') & VaR.plot=='TRUE')  print(lines(VaRUp.series[,i],      col="black", lwd=1.5,lty='dashed'))
    if (CoVaR.plot=='Up'|CoVaR.plot=='Both')                       print(lines(DeltaCoVaRUp.series[,i],col="blue",lwd=1.5,lty='solid'))
    if (VaR.plot=='TRUE')                                          print(addLegend("topright", lwd=2, legend.names = c('CoVaR', 'VaR',   'DeltaCoVaR'), 
                                                                                   lty = c('dotted','dashed','solid'), col = c('red',   'black', 'blue')))
    else    print(addLegend("topright", lxwd=2, legend.names = c('CoVaR', 'DeltaCoVaR'), 
